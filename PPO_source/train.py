...
def train():
    ...
    K_epochs = 7               # update policy for K epochs in one PPO update
    ...
    while time_step <= max_training_timesteps:
        ...
        i_episode += 1
        if len(done_arr) > 50 and np.mean(done_arr[-50:])>0.85 and np.mean(done_arr[-20:])>850:
