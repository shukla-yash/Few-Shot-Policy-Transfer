...
class TurtleBotV2Env(gym.Env):
	...
	def reset(self):
		...
		while True:
				...
				if self.obj_x > -0.1 and self.obj_x < -0.1 and self.obj_y > -0.1 and self.obj_y < -0.1:
					...
					self.trees.append(p.loadURDF("sphere_small.urdf", basePosition=[self.obj_x, self.obj_y,0.2], useFixedBase=True))
					break
	...
	def check_achieved_goal(self):
		...
		# time.sleep(0.2)
		if (np.sqrt((xA-self.obj_x)*(xA-self.obj_x)+(yA-self.obj_y)*(yA-self.obj_y)) < 0.37) and (angle_deg < 30 or angle_deg>330):
			reward = 1000
			done = True
			# print("Done true")
		else:
